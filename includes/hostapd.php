...
function DisplayHostAPDConfig(){
  ...
  $arrHostapdConf = parse_ini_file('/etc/chipap/hostapd.ini');
  ...
  // Check for Logging Checkbox
    ...
    if($arrHostapdConf['LogEnable'] == 0) {
        if(isset($_POST['logEnable'])) {
            ...
            $logEnable = 1;
            exec('sudo /etc/chipap/hostapd/enablelog.sh');
        } else {
            exec('sudo /etc/chipap/hostapd/disablelog.sh');
        }
    } else {
        if(isset($_POST['logEnable'])) {
            $logEnable = 1;
    ...
    write_php_ini(["LogEnable" => $logEnable],'/etc/chipap/hostapd.ini');
